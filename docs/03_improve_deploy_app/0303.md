...
2. Review the workflow contents. Then, under the "Setup .NET Core" step, ensure that the .NET version is `8.0.x` in order to match the version defined in the Team Messaging System.
3. Ensure that the workflow is configured to trigger on both pushes and pull requests, and then configure both of these triggers with path triggers which only act on changes in the `/src/Application` folder. Use GitHub Copilot to assist you in this step.
...
5. Build a Docker file called `Dockerfile` in the folder `/src/Application/src/RazorPagesTestSample`, using GitHub Copilot to assist you in developing the file. This Dockerfile should target a .NET 8 project with an entry point of `RazorPagesTestSample.dll`.

    {: .note }
    > The default ASP.NET port has changed from 80 to 8080 as of .NET 8.0. You will want to set the environment variable `ASPNETCORE_HTTP_PORTS=80` in your Dockerfile.

6. Test the workflow by making a small change to application code, such as adding a comment to a file. Commit and push the change, and then ensure that the workflow completes successfully. At this point, any changes pushed to the `/src/Application` folder will automatically trigger the workflow--this is Continuous Integration in action.
7. At the top of your workflow file, create 4 environment variables:
    ...
    - `repositoryName` - The repository to target in the registry. Set this to "`techexcel/dotnetcoreapp`".
    - `dockerFolderPath` - The path to the folder that contains the Dockerfile. You will need to point to the folder `/src/Application/src/RazorPagesTestSample`.
...
- Any changes pushed to the `/src/Application` folder automatically trigger the workflow.
...
- Use GitHub Copilot to generate GitHub Actions workflows. If you wish to compare your results versus one solution, you can find solution files for the GitHub Actions workflow in the solutions folder. To make the solution easier to understand, there are several versions of the solution file. Each version builds upon the prior, so the final version covers all parts of the solution.
  - The [first version of the solution file](https://github.com/microsoft/TechExcel-Accelerate-developer-productivity-with-GitHub-Copilot-and-Dev-Box/blob/main/Solution/Exercise-03/Task-3/dotnet-deploy-1.yml) covers steps 1-5.
  - The [second version of the solution file](https://github.com/microsoft/TechExcel-Accelerate-developer-productivity-with-GitHub-Copilot-and-Dev-Box/blob/main/Solution/Exercise-03/Task-3/dotnet-deploy-2.yml) adds in steps 6-12.
  - The [third version of the solution file](https://github.com/microsoft/TechExcel-Accelerate-developer-productivity-with-GitHub-Copilot-and-Dev-Box/blob/main/Solution/Exercise-03/Task-3/dotnet-deploy-3.yml) adds in steps 13-15.
...
{: .note }
> The `images` command in each `webapps-deploy` call has a hard-coded value for the image location of `techexcel/dotnetcoreapp` and expect you to hard-code the ACR location. This is because the `images` command will not accept repository secrets or variables defined in the workflow like the steps in the `dockerBuildPush` job do.

- The solution for a Dockerfile in this task is [in the solutions folder](https://github.com/microsoft/TechExcel-Accelerate-developer-productivity-with-GitHub-Copilot-and-Dev-Box/blob/main/Solution/Exercise-03/Task-3/Dockerfile).
...
- The [fourth version of the solution file](https://github.com/microsoft/TechExcel-Accelerate-developer-productivity-with-GitHub-Copilot-and-Dev-Box/blob/main/Solution/Exercise-03/Task-3/dotnet-deploy-4.yml) adds in the advanced challenge.
